terraform {
  ...
  backend "gcs" {
    bucket = "terraform-state-1cb0jrpk"
...
resource "google_project_iam_member" "secretmanager_secretAccessor_role" {
  project = data.google_client_config.current.project
...
resource "google_container_cluster" "disearch_cluster" {
  ...
  depends_on = [ google_project_service.enable_container, null_resource.global_delay, google_service_account.workload_identity_user_sa]
